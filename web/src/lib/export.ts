...
function createDocxFromMarkdown(markdown: string, title: string): Document {
  ...
  // Helper to flush the current table
  const flushTable = () => {
  ...
  for (const line of lines) {
    ...
    // If we were in a table but hit non-table content, flush the table
    if (inTable) {
      flushTable();
    }
    // Heading 1
  ...
  // Flush any remaining table at the end of the document
  if (inTable) {
    flushTable();
  }
...
function renderPdfTable(
  ...
  // Calculate column widths to fit within contentWidth
  ...
  autoTable(doc, {
    startY: yPos,
    margin: { left: margin, right: margin },
    tableWidth: contentWidth, // Constrain table to page width
    ...
    columnStyles: Object.fromEntries(
      Array.from({ length: numCols }, (_, i) => [i, { cellWidth: colWidth }])
    ),
    styles: {
      overflow: 'linebreak',
      cellWidth: 'wrap',
